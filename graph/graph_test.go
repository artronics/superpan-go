...
import (
	...
	"os"
	"testing"
	"github.com/artronics/superpan/graph/testgraph"
)
...
func TestNewGraphFromJSON_graph(t *testing.T) {
	...
	if g.vertexToTargets[StringID("C")][StringID("S")] != 9.0 {
		t.Fatalf("weight from C to S must be 9.0 but %f", g.vertexToTargets[StringID("C")][StringID("S")])
	}
	for _, tg := range testgraph.GraphSlice {
		...
		if g.GetVertexCount() != tg.TotalVertexCount {
			t.Fatalf("%s | Expected %d but %d", tg.Name, tg.TotalVertexCount, g.GetVertexCount())
		}
		for _, elem := range tg.EdgeToWeight {
			weight1, err := g.GetWeight(StringID(elem.Vertexs[0]), StringID(elem.Vertexs[1]))
			if err != nil {
				t.Fatal(err)
			}
			weight2 := elem.Weight
...
func TestGraph_DeleteVertex(t *testing.T) {
	...
	if !g.DeleteVertex(StringID("D")) {
		t.Fatal("D does not exist in the graph")
	}
	if g.GetVertex(StringID("D")) != nil {
		t.Fatalf("Expected Vertex but %s", g.GetVertex(StringID("D")))
	}
